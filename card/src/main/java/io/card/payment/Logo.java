...
class Logo {
    ...
    void loadLogo(boolean useCardIOLogo) {
        ...
        if (useCardIOLogo) {
            mLogo = ViewUtil.base64ToBitmap(Base64EncodedImages.paypal_sdk_icon_mastercard_large, mContext, density);
        } else {
            mLogo = ViewUtil.base64ToBitmap(Base64EncodedImages.paypal_sdk_icon_visa_large, mContext, density);
        }
    }
